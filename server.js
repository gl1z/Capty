...
// stricter limit for ai endpoints since they cost money
...
app.post('/api/whisper', aiLimiter, async (req, res) => {
    ...
    try {
        // quality 9 = smallest mp3, good enough for speech
        // deno runtime needed for yt-dlp to handle youtube's js properly
        await runYtDlp([
            '--extract-audio',
            '--audio-format', 'mp3',
            '--audio-quality', '9',
        ...
        if (fileSize > 25 * 1024 * 1024) {
            ...
            return res.status(400).json({ error: 'Audio too large for Whisper (max 25MB). Try a shorter video.' });
        }

        console.log(`[whisper] sending ${(fileSize / 1024 / 1024).toFixed(2)}mb to openai`);
        ...
        fs.unlinkSync(audioPath);
        console.log(`[whisper] done — ${result.length} chars`);
